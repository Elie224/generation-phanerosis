...
<div class="space-y-6">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <h1 class="text-2xl font-bold text-gray-900">Modifier l'enseignement</h1>
            <p class="mt-1 text-sm text-gray-600">Modifiez les informations de cet enseignement.</p>
        </div>
    </div>
    ...
    <div class="bg-white shadow-sm sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            ...
            <form action="{{ route('admin.teachings.update', $teaching->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $teaching->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>

                    <div>
                        <label for="pastor" class="block text-sm font-medium text-gray-700">Pasteur/Prédicateur *</label>
                        <input type="text" name="pastor" id="pastor" value="{{ old('pastor', $teaching->pastor) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                </div>

                <div class="mt-6">
                    <label for="teaching_date" class="block text-sm font-medium text-gray-700">Date de l'enseignement *</label>
                    <input type="date" name="teaching_date" id="teaching_date" value="{{ old('teaching_date', $teaching->teaching_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                </div>

                <div class="mt-6">
                    <label for="content" class="block text-sm font-medium text-gray-700">Contenu de l'enseignement *</label>
                    <textarea name="content" id="content" rows="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('content', $teaching->content) }}</textarea>
                    ...
                    <div>
                        ...
                        @if($teaching->media_path)
                            <div class="mb-2">
                                ...
                                <a href="{{ Storage::url($teaching->media_path) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 text-sm">
                                    {{ basename($teaching->media_path) }}
                                </a>
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        @if($teaching->image_path)
                            <div class="mb-2">
                                ...
                                <img src="{{ Storage::url($teaching->image_path) }}" alt="Image actuelle" class="w-20 h-20 object-cover rounded">
                            </div>
                        @endif
