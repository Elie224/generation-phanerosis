        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-900">Modifier l'enseignement</h2>
                    <a href="{{ route('admin.teachings') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        Retour à la liste
                    </a>
                </div>

                <form action="{{ route('admin.teachings.update', $teaching) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700">Titre de l'enseignement</label>
                        <input type="text" name="title" id="title" value="{{ old('title', $teaching->title) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="10" required
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('content', $teaching->content) }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pastor" id="pastor" value="{{ old('pastor', $teaching->pastor) }}" required
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        @error('pastor')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="teaching_date" id="teaching_date" value="{{ old('teaching_date', $teaching->teaching_date) }}" required
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        @error('teaching_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        @if($teaching->media_path)
                            <p class="mt-1 text-sm text-green-600">Fichier actuel: {{ basename($teaching->media_path) }}</p>
                        @endif
                        @error('media_file')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        @if($teaching->image_path)
                            <p class="mt-1 text-sm text-green-600">Image actuelle: {{ basename($teaching->image_path) }}</p>
                        @endif
                        @error('image_file')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
